...
# Install packages
sudo NEEDRESTART_MODE=a DEBIAN_FRONTEND=noninteractive apt install -y python3.12-full \
    python-is-python3 gcc zsh make git apt-transport-https docker-ce docker-ce-cli \
    containerd.io docker-buildx-plugin docker-compose-plugin net-tools stow tree \
    unzip python3-pip vault xclip adb fastboot xmlformat-perl shellcheck shfmt
...
npm install -g prettier bash-language-server markdownlint-cli
...
if [[ -f "$UV_TOOLS_FILE" ]]; then
    ~/.local/bin/yq -r '.tools[]' "$UV_TOOLS_FILE" | while read -r tool; do
        ~/.local/bin/uv tool install "$tool"
    done
fi
